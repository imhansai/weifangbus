class StartupbasicinfoEntity {
	...
	StartupbasicinfoEntity.fromJson(Map<String, dynamic> json) {
		if (json['HeadLine'] != null) {
			...
			json['HeadLine'].forEach((v) { headline.add(new Headline.fromJson(v)); });
		}
		if (json['SlideShow'] != null) {
			slideshow = new List<Slideshow>();
			json['SlideShow'].forEach((v) { slideshow.add(new Slideshow.fromJson(v)); });
		}
		if (json['LoginPic'] != null) {
			loginpic = new List<Loginpic>();
			json['LoginPic'].forEach((v) { loginpic.add(new Loginpic.fromJson(v)); });
		}
	}
...
class Headline {
	...
	Headline({this.id, this.title, this.content, this.realeasetime, this.routeid, this.stationid, this.realeasedatetime, this.contenthtml, this.istop1, this.isscroll});
